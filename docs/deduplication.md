...
> **说明**：本文档为历史记录。当前同步不再使用 `sync-state` 分支持久化状态；去重脚本仅在历史上已产生重复文章时使用。
...
创建了 `tools/deduplicate_cnblogs.py` 脚本，具备以下功能：
...
- `src/assemble_publish/sync_to_cnblogs.py` - 同步脚本（需要修复重复创建问题）
- `tools/deduplicate_cnblogs.py` - 去重工具
